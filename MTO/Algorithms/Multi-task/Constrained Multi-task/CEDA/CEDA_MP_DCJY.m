classdef CEDA_MP_DCJY < Algorithm
% <Multi-task> <Single-objective> <Constrained>
%
% Dual-channel transfer variant with elite mapping:
% In the feasible channel, only top individuals ranked by Obj are used
% to build the domain adaptation mapping, biasing transfer toward
...
methods
    function Parameter = getParameter(Algo)
        Parameter = {'EC_Top', num2str(Algo.EC_Top), ...
                ...
                'EliteRatio (feasible channel)', num2str(Algo.EliteRatio)};
    end
    ...
    function Algo = setParameter(Algo, Parameter)
        ...
        Algo.EliteRatio = str2double(Parameter{i}); i = i + 1;
    end
    ...
    function [temp_Dec, temp_Dec2] = BuildChannelMaps(Algo, population, transpop, use_feasible_channel)
        % Source distribution: all individuals in this channel (for accurate estimation)
        dst_all = Algo.SelectChannelPopulation(population, use_feasible_channel, false);
        src_all = Algo.SelectChannelPopulation(transpop, use_feasible_channel, false);
        if use_feasible_channel
            % Target distribution: elite subset (to bias mapping toward promising region)
            dst_elite = Algo.SelectChannelPopulation(population, true, true);
            src_elite = Algo.SelectChannelPopulation(transpop, true, true);
            % Map from full source distribution to elite target distribution
            temp_Dec = CEDA_trans(src_all, dst_elite, transpop.Decs);
            temp_Dec2 = CEDA_trans(dst_all, src_elite, population.Decs);
        else
            temp_Dec = CEDA_trans(src_all, dst_all, transpop.Decs);
            temp_Dec2 = CEDA_trans(dst_all, src_all, population.Decs);
        end
    end

    function chosen = SelectChannelPopulation(Algo, pop, use_feasible_channel, elite_filter)
        if nargin < 4
            elite_filter = false;
        end
        ...
        if use_feasible_channel
            idx = find(cv <= 0);
            % Elite filtering: keep only top individuals by Obj
            if elite_filter && numel(idx) >= 2
                obj_feas = [pop(idx).Obj];
                [~, obj_order] = sort(obj_feas, 'ascend');
                n_elite = max(2, ceil(Algo.EliteRatio * numel(idx)));
                idx = idx(obj_order(1:n_elite));
            end
        else
            idx = find(cv > 0);
        end
