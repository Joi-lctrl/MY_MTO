classdef CEDA_MP_DUALCHANNEL_BLEND < Algorithm
...
% 3) mapped solutions are blended with original solutions before variation.
...
methods
    function Parameter = getParameter(Algo)
        Parameter = {'EC_Top', num2str(Algo.EC_Top), ...
                ...
                'BlendAlpha', num2str(Algo.BlendAlpha)};
    end
    ...
    function Algo = setParameter(Algo, Parameter)
        ...
        Algo.BlendAlpha = str2double(Parameter{i}); i = i + 1;
    end
    ...
    function offspring = Generation1(Algo, population, pool, transpop)
        ...
        for i = 1:length(population)
            if rand() < Algo.RMP1
                ...
                population(i).Dec = Algo.BlendDec(origin_dec, mapped_dec);
            end
        end
    ...
    function dec = BlendDec(Algo, origin_dec, mapped_dec)
        alpha = min(max(Algo.BlendAlpha, 0), 1);
        dec = alpha * mapped_dec + (1 - alpha) * origin_dec;
        dec(dec > 1) = 1;
        dec(dec < 0) = 0;
    end
end
