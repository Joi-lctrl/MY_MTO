classdef CEDA_MP_DUALCHANNEL_EP < Algorithm
% <Multi-task> <Single-objective> <Constrained>
%
% Dual-channel transfer variant with epsilon-relaxed channel partitioning:
% 1) epsilon-feasible individuals (CV <= Ep) use the feasible mapping channel;
% 2) epsilon-infeasible individuals (CV > Ep) use the infeasible mapping channel.
%
% This aligns the channel routing with the epsilon-constraint relaxation
% used in mating selection and survival selection.
...
methods
    ...
    function offspring = Generation1(Algo, population, pool, transpop, Ep)
        ...
        [temp_Dec_feas, temp_Dec2_feas] = Algo.BuildChannelMaps(population_temp, transpop, Ep, true);
        [temp_Dec_infeas, temp_Dec2_infeas] = Algo.BuildChannelMaps(population_temp, transpop, Ep, false);

        for i = 1:length(population)
            if rand() < Algo.RMP1
                if population(i).CV <= Ep
                    if rand() < 0.5
    ...
    function [temp_Dec, temp_Dec2] = BuildChannelMaps(Algo, population, transpop, Ep, use_feasible_channel)
        dst_pop = Algo.SelectChannelPopulation(population, Ep, use_feasible_channel);
        src_pop = Algo.SelectChannelPopulation(transpop, Ep, use_feasible_channel);
        temp_Dec = CEDA_trans(src_pop, dst_pop, transpop.Decs);
        temp_Dec2 = CEDA_trans(dst_pop, src_pop, population.Decs);
    end

    function chosen = SelectChannelPopulation(~, pop, Ep, use_feasible_channel)
        cv = [pop.CV];
