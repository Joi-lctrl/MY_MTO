classdef CEDA_MP_DUALCHANNEL_FEASELITE < Algorithm
...
% 3) the feasible channel is built from feasible elite subsets only.
...
methods
    function Parameter = getParameter(Algo)
        Parameter = {'EC_Top', num2str(Algo.EC_Top), ...
                ...
                'FeasEliteRatio', num2str(Algo.FeasEliteRatio)};
    end
    ...
    function Algo = setParameter(Algo, Parameter)
        ...
        Algo.FeasEliteRatio = str2double(Parameter{i}); i = i + 1;
    end
    ...
    function [temp_Dec, temp_Dec2] = BuildChannelMaps(Algo, population, transpop, use_feasible_channel)
        ...
        if use_feasible_channel
            dst_pop = Algo.SelectFeasibleElites(dst_pop);
            src_pop = Algo.SelectFeasibleElites(src_pop);
        end
    ...
    function elite_pop = SelectFeasibleElites(Algo, pop)
        obj = [pop.Obj];
        take = min(length(pop), max(2, ceil(Algo.FeasEliteRatio * length(pop))));
