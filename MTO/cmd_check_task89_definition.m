...
for task_id = [8 9]
    ...
    assert(numel(strfind(mac, 'LESIZE, ALL, , , 50')) == 9, ...
        'Task%d should create 9 rib rows with mesh size 50.', task_id);
    assert(contains(mac, 'ESEL, S, SEC, , 5, 9'), ...
        'Task%d should select rib sections for SFBEAM loading.', task_id);
    assert(contains(mac, sprintf('SFBEAM, ALL, 1, PRES, %g, %g', task.q1, task.q1)), ...
        'Task%d should use the original SFBEAM face-1 rib load.', task_id);
