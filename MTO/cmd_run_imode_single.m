%% cmd_run_imode_single.m - 逐任务单独运行IMODE并保存统一格式结果
...
%% ===== 图形 =====
fig1 = figure('Name', 'IMODE 单任务结构优化收敛', 'Position', [60 450 900 400]);
ax = gobjects(1, nTask);
...
for rep = 1:Reps
    ...
    for task_slot = 1:nTask
        ...
        if isempty(task_fe_gen)
            error('IMODE Task %d 未生成 FE_Gen，无法构造收敛曲线。', actual_task);
        end
...
%% ===== 保存 =====
save_name = sprintf('imode_single_%s.mat', datestr(now, 'yyyymmdd_HHMMSS'));
...
if show_gen_progress && current_gen > es('last_gen_printed')
    fprintf('    [IMODE][Task %d] Gen %d, FE=%d, BestObj=%.6f, BestCV=%.6f\n', ...
