%% Quick MFEA-II RMP test on ship panel problem
% Usage:
%   1. cd('MTO');
%   2. run('cmd_ship_rmp_test.m');
...
save_rmp_log = true;
rmp_log_dir = fullfile(pwd, 'RMPLogs', 'cmd_ship_rmp_test');

fprintf('=== MFEA-II RMP Test ===\n');
fprintf('Pop size: %d, Target gen: %d, Total FE: %d\n', ...
    pop_size, target_generations, total_fe);

%% Algorithm
mfea_ii = MFEA_II_RMPLog();
mfea_ii.Name = 'MFEA-II';
...
if save_rmp_log
    save_info = save_rmp_history(mfea_ii, active_tasks, rmp_log_dir, ...
        'cmd_ship_rmp_test');
...
%% Plot RMP
if ~isempty(mfea_ii.RMPHistory)
    figure('Name', 'RMP Evolution');
    rmp_hist = mfea_ii.RMPHistory;
    rmp_gen = mfea_ii.RMPHistoryGen;
...
if isprop(mfea_ii, 'TransferDiagHistory') && ~isempty(mfea_ii.TransferDiagHistory)
    summarize_transfer_diag(mfea_ii, active_tasks);
end
